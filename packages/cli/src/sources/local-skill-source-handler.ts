 ...
 */
export async function resolveLocalSource(source: string): Promise<SkillSource> {
  ...
  if (stats.isDirectory()) {
    ...
    if (!isValid) {
      throw new Error(
        `Directory does not appear to be a skill: ${source}\n` +
 ...
 */
async function validateSkillDirectory(dirPath: string): Promise<boolean> {
  try {
    const files = await readdir(dirPath);
    return SKILL_INDICATORS.some((indicator) =>
      files.some((f) => f.toLowerCase() === indicator.toLowerCase())
    );
  } catch {
    return false;
  }
 ...
 */
export async function findTestsInSkillDir(skillPath: string): Promise<string[]> {
